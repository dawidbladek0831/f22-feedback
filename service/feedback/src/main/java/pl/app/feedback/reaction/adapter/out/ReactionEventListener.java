...
public class ReactionEventListener {
    ...
    @Bean
    public Consumer<ReactionEvent.ReactionAddedEvent> reactionAddedEvent() {
        return event -> {
            logger.debug("received event {}, value: {}", event.getClass().getSimpleName(), event);
    ...
    @Bean
    public Consumer<ReactionEvent.ReactionRemovedEvent> reactionRemovedEvent() {
        return event -> {
            logger.debug("received event {}, value: {}", event.getClass().getSimpleName(), event);
            updateDomainObjectReaction(event.domainObjectType(), event.domainObjectId(), event.reaction(), -1).subscribe();
    ...
    public Mono<UserReaction> updateUserReaction(String userId, String domainObjectType, String domainObjectId, String reactionType, boolean add) {
