...
class KafkaEventPublisher implements EventPublisher {
    ...
    public Mono<Void> publish(Object event) {
        ...
        return switch (event) {
            ...
            case RatingEvent.RatingCreatedEvent e -> Mono.fromFuture(kafkaTemplate.send(topicNames.getRatingCreated().getName(), e.id(), event)).then();
            case RatingEvent.RatingUpdatedEvent e -> Mono.fromFuture(kafkaTemplate.send(topicNames.getRatingUpdated().getName(), e.id(), event)).then();
            case RatingEvent.RatingRemovedEvent e -> Mono.fromFuture(kafkaTemplate.send(topicNames.getRatingRemoved().getName(), e.id(), event)).then();
